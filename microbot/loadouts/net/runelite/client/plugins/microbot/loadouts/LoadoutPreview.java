...
import net.runelite.client.plugins.microbot.loadouts.ui.LoadoutItemTooltip;
...
public class LoadoutPreview
{
	...
	private static void renderWithSlotMenu(JComponent comp, PreviewIconSource icons, String fileName) throws Exception
	{
		...
		SwingUtilities.invokeAndWait(() ->
		{
			...
			final net.runelite.client.plugins.microbot.loadouts.ui.ItemSummaryPanel head =
					new net.runelite.client.plugins.microbot.loadouts.ui.ItemSummaryPanel(
	...
	{
		// save()/load() are never triggered in the preview, so the null config
		// store is harmless; the panel only reads getLoadouts().
		return new LoadoutManager(null, new Gson());
	}
